...
- 一時停止中に 1 秒単位の早送り / 巻き戻し
- 一時停止 UI にコマ送り・1 秒送り・撮影ボタンを追加
- フルスクリーン再生中も全機能が動作
...
コマ送りだけでは目的の位置まで遠く、再生中の 10 秒送りでは粗すぎるため、
その中間として 1 秒送りを用意している。

一時停止すると、既存の再生コントロールの下に 5 つのボタン
(1 秒戻す / コマ戻す / 撮影 / コマ送る / 1 秒送る) が表示される。

保存先は Chrome のダウンロードフォルダ配下の
`z-an/<番組タイトル>_<再生位置>.png` (macOS なら `~/Downloads/z-an/`、
Windows なら `%USERPROFILE%\Downloads\z-an\`)。再生位置は
`HH-MM-SS.mmm` 形式なので、連写してもファイル名が衝突せず、
ファイル名順に並べると時系列順になる。

ホットキーと保存先は、拡張機能ページ (`⋮` →「拡張機能」→「拡張機能を管理」) で
この拡張の「詳細」を開き、「拡張機能のオプション」から変更できる。
...
ダウンロードフォルダは整理のときに消しやすいので、`書類` (Windows なら `ドキュメント`)
など普段触らない場所に置いておくとよい。
...
1. Chrome の拡張機能ページを開く
   - 右上の `⋮` →「拡張機能」→「拡張機能を管理」
   - または、アドレスバーに `chrome://extensions` と入力する
...
`zip` コマンドが要る。Git for Windows には同梱されていないため、Windows で
ローカル ZIP を作るなら別途入れる (リリース自体は GitHub Actions 側で作られる)。
...
| `src/content/stepper.js` | fps 実測とシーク (コマ送り / 1 秒送り) |
