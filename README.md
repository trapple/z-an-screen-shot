...
- **ポインタ系イベントは `document` の capture フェーズで `stopImmediatePropagation`
  ...
  止めても間に合わない。バーごとにリスナを張らずに済むよう `document` にまとめ、
  対象がバーの中かどうかで振り分けている
- **バーのボタンとホットキーは、操作の前後で再生状態が変わらないように戻している。**
  ブラウザの自動再生ポリシーにより、ページに最初のユーザー操作が入った時点で、
  保留されていた再生が解禁されて動き出す。こちらの操作もその「操作」に数えられるため、
  F5 直後の未再生状態でボタンを 1 回押すと、それだけで再生が始まってしまう。
  `preventDefault` も `stopImmediatePropagation` もジェスチャの成立自体は妨げないので、
  イベントを止める方向では直らない。`index.js` の `holdPlayState` が、操作の直後
  300ms だけ `play` を監視して元に戻す。**この症状を「クリックがすり抜けている」と
  読むと延々と外すので注意** (プレイヤーの外をクリックしても同じことが起きる、
  で切り分けられる)
